...
year_dir = os.path.join(Climate_data_dir, '2010_data')
...
#filelist.reverse()
# looks like Jan, Feb, March are missing data from a station
#a = sel_NOAA_data.sort_values(by =['WBAN'])
#b = sel_NOAA_data_all.sort_values(by = ['WBAN'])
#a[~a['WBAN'].isin(b['WBAN'])] #WBAN 73805
# In[3]:
...
for f, file in enumerate(filelist):
    ...
    WB = 'WB_' + str(file[4:6])
    WS = 'WS_' + str(file[4:6])
    sel_NOAA_data[DB] = (sel_NOAA_data['AvgTemp'].astype(float) - 32)*(5/9) # convert farenheit to celcius
    sel_NOAA_data[WB] = (sel_NOAA_data['AvgWetBulb'].astype(float) - 32)*(5/9) # convert farenheit to celcius
    sel_NOAA_data[WS] = sel_NOAA_data['AvgWindSpeed'].astype(float)
    print(len(sel_NOAA_data))
    
    if f == 0:
        sel_NOAA_data_all = sel_NOAA_data.copy(deep = True)
    else: 
        sel_NOAA_data_all = sel_NOAA_data_all.merge(sel_NOAA_data, how ='right',
                                                    left_on = 'WBAN', right_on = 'WBAN')
...
NOAA_idw.to_csv('2010_processed_NOAA_data.csv')
#%%
