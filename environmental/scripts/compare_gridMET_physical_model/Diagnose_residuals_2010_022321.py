...
pm_2015_ev = pm_2015.copy(deep = True)
pm_2015_ev.drop(columns = ['COUNTY', 'STATE', 'NAME_OF_WATER_SOURCE','COOLING_TYPE', 'MODEL_TYPE', 'PERCENT_CD_ALLOCATION', 
...
pm_2010 = pm_2010.merge(pm_2015_ev,how = 'left', left_on ='Plant_ID', right_on = 'EIA_PLANT_ID')
# 2.1 make rows into columns for input_2015 data
plants = pm_2010['Plant_ID']
cols = ['DB','WB','WS','EV']

#%% from the 2015 script
...
plants = pm_2015['EIA_PLANT_ID']
cols = ['DB','WB','WS','EV']

for i, EIA in enumerate(plants):
    temp = pm_2015[pm_2015['EIA_PLANT_ID'] == EIA]
...
#if you want to run this for 2015, change here to pm_2015
for i, EIA in enumerate(plants):
...
#%%
#2015
pm_2015 =df2.copy(deep=True)
pm_2015 = pm_2015[['EIA_PLANT_','month','DB', 'WB', 'WS', 'EV']]
#conv Evap from inches to mm
pm_2015['EV'] = pm_2015['EV']*conv_mm
#conv WS from mph to m/s
pm_2015['WS'] = pm_2015['WS']*conv_ms

#rename cols
cols = ['EIA_PLANT_','month','DB_C', 'WB_C', 'WS_ms', 'EV_mm']
pm_2015.columns = cols
...
#%%
month_means_15['EIA_P_DATE'] = month_means_15.apply(lambda row: str(row.EIA_PLANT_) + "_2015-" + str(row.month), axis = 1)
month_medians_15['EIA_P_DATE'] = month_medians_15.apply(lambda row: str(row.EIA_PLANT_) + "_2015-" + str(row.month), axis = 1)
pm_2015['EIA_P_DATE'] = pm_2010.apply(lambda row: str(row.EIA_PLANT_) + "_2015-" + str(row.month), axis = 1)
...
#rename cols
cols_mean =['EIA_PLANT_','month','gm_DB_mean', 'gm_EV_mean', 'gm_WB_mean','gm_WS_mean','EIA_P_DATE']
month_means_15.columns = cols_mean
cols_median =['EIA_PLANT_','month','gm_DB_med', 'gm_EV_med', 'gm_WB_med','gm_WS_med','EIA_P_DATE']
month_medians_15.columns = cols_median

#reorder cols
month_means_15 = month_means_15[['EIA_P_DATE','EIA_PLANT_','month','gm_DB_mean', 'gm_WB_mean', 'gm_WS_mean', 'gm_EV_mean']]
month_medians_15 = month_medians_15[['EIA_P_DATE','EIA_PLANT_','month','gm_DB_med', 'gm_WB_med', 'gm_WS_med', 'gm_EV_med']]
#pm_2010 = pm_2010[['EIA_P_DATE','EIA_PLANT_','month', 'DB_C','WB_C','WS_ms','EV_mm']]

# In[20]:


len(month_means_15) == 1106*12
len(month_medians_15) == 1106*12
...
#%% 2015
mean_stats = pd.merge(month_means_15, pm_2015, on = 'EIA_P_DATE')
