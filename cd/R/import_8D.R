import_8D<-function(cooling8D){
  ...
  #Test variable types
  test_that("8D cooling data variable types are correct",
            {
              expect_true(is.numeric(cooling8D$Plant.Code), "Error wrong variable type. Plant.Code is a numeric variable.")
              expect_true(is.character(cooling8D$Cooling.ID), "Error wrong variable type. Cooling.ID is a character string.")
              expect_true(is.numeric(cooling8D$Withdrawal_GPM), "Error wrong variable type. Withdrawal_GPM is a numeric variable.")
              expect_true(is.numeric(cooling8D$Consumption_GPM), "Error wrong variable type. Consumption_GPM is a numeric variable.")
