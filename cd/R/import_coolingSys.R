...
import_coolingSys <- function(cooling)
{
  ...
  test_that("Cooling data variable types are correct",
            {
              expect_true(is.numeric(cooling$Plant.Code), "Error wrong variable type. Plant.Code is a numeric variable.")
              expect_true(is.character(cooling$Cooling.ID), "Error wrong variable type. Cooling.ID is a character string.")
              expect_true(is.character(cooling$Cooling.Type.1), "Error wrong variable type. Cooling.Type.1 is a character string.")
              expect_true(is.character(cooling$Cooling.Type.2), "Error wrong variable type. Cooling.Type.2 is a character string.")
