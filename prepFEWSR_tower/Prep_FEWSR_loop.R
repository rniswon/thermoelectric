...
setwd("C:/Users/lgorman/OneDrive - DOI/Repos/Thermoelectric/") #top of repo, change as needed

#path to fewsr_tower_input files
fewsrDataPath<-"D:/FEWSRinput/lowWTremoved/updated_fewsr_tower_input/"
#outputpath for prepped fewsr files
outputData_path<-paste0(fewsrDataPath,"FEWSR_input/")

#read in plant pond elevation info to join to CD data
plant_pond_info <- read.csv("C:/Users/lgorman/OneDrive - DOI/Repos/Thermoelectric/cd/Data/plant_elev_pond_info_aeg.csv", header = T, stringsAsFactors = F)
...
fewsr_prep <- function(dfout){
  ...
  dfout <- dfout %>% select(Plant.Code, elevation, pond_area, 
                            ...
                            contains("WT_"), contains("WS_"),contains("NatEV"))
...
for (i in 1:length(years)) {
  ...
  CD_data.i <- read.csv(paste(fewsrDataPath,years[i],"_fewsr_tower_input_final.csv", sep =""), header = T, stringsAsFactors = F)
  ...
  for (t in types){
    ...
    #plant 6035 needs double pond acre
    if (year.i>=2015){
      df <- df %>% dplyr::mutate(pond_area=ifelse(Plant.Code==6035,pond_area*2,pond_area))
    }
    
    #natural eval calculation
    #convert from acre-in/day to MG/day
    df<-df %>% dplyr::mutate(NatEV_jan = pond_area*EV_jan*6272640*0.004329/1000000,
                             NatEV_feb = pond_area*EV_feb*6272640*0.004329/1000000,
                             NatEV_mar = pond_area*EV_mar*6272640*0.004329/1000000,
                             NatEV_apr = pond_area*EV_apr*6272640*0.004329/1000000,
                             NatEV_may = pond_area*EV_may*6272640*0.004329/1000000,
                             NatEV_jun = pond_area*EV_jun*6272640*0.004329/1000000,
                             NatEV_jul = pond_area*EV_jul*6272640*0.004329/1000000,
                             NatEV_aug = pond_area*EV_aug*6272640*0.004329/1000000,
                             NatEV_sep = pond_area*EV_sep*6272640*0.004329/1000000,
                             NatEV_oct = pond_area*EV_oct*6272640*0.004329/1000000,
                             NatEV_nov = pond_area*EV_nov*6272640*0.004329/1000000,
                             NatEV_dec = pond_area*EV_dec*6272640*0.004329/1000000)
