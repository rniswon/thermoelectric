...
# read in condenser duty and environmental data
  CD_data.i <- read.csv(paste("./cd/Data/",years[i],"_fewsr_tower_input.csv", sep =""), header = T, stringsAsFactors = F)
  ...
  final_tower_input <- tower.input1 %>% select(Plant.Code, Elevation, Pond_Area, 
  ...
  final_tower_input<-final_tower_input %>% select(Plant.Code, Elevation, Pond_Area,Tdb, Twb, nwT)
  #put it back together
  final_tower_input<-cbind(final_tower_input,towerData)
  #organize in correct order
  final_tower_input <- select(Plant.Code, Elevation, Pond_Area, 
                              contains("CD_"), contains("DB_"), contains("WB_"),
                              contains("WT_"), contains("WS_"), Tdb, Twb, nwT)
