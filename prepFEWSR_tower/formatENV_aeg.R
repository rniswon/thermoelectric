...
dataPath<-"C:/Users/galanter/DOI/GS-WMA-WBEEP - Thermoelectric/Water_Temps/"
...
for (y in years){
  ...
  df.out<-df.out %>% rowwise() %>% dplyr::mutate(WT_jan = ifelse(WT_jan==-99.9 | is.na(WT_jan),
                                                                 DB_jan,WT_jan),
                                                 WT_feb = ifelse(WT_feb==-99.9 | is.na(WT_feb),
                                                                 DB_feb,WT_feb),
                                                 WT_mar = ifelse(WT_mar==-99.9 | is.na(WT_mar),
                                                                 DB_mar,WT_mar),
                                                 WT_apr = ifelse(WT_apr==-99.9 | is.na(WT_apr),
                                                                 DB_apr,WT_apr),
                                                 WT_may = ifelse(WT_may==-99.9 | is.na(WT_may),
                                                                 DB_may,WT_may),
                                                 WT_jun = ifelse(WT_jun==-99.9 | is.na(WT_jun),
                                                                 DB_jun,WT_jun),
                                                 WT_jul = ifelse(WT_jul==-99.9 | is.na(WT_jul),
                                                                 DB_jul,WT_jul),
                                                 WT_aug = ifelse(WT_aug==-99.9 | is.na(WT_aug),
                                                                 DB_aug,WT_aug),
                                                 WT_sep = ifelse(WT_sep==-99.9 | is.na(WT_sep),
                                                                 DB_sep,WT_sep),
                                                 WT_oct = ifelse(WT_oct==-99.9 | is.na(WT_oct),
                                                                 DB_oct,WT_oct),
                                                 WT_nov = ifelse(WT_nov==-99.9 | is.na(WT_nov),
                                                                 DB_nov,WT_nov),
                                                 WT_dec = ifelse(WT_dec==-99.9 | is.na(WT_dec),
                                                                 DB_dec,WT_dec))
 ...
 # --> change to monthly DB
  ...
  df.out.winter<-df.out %>% dplyr::mutate(DB_jan = ifelse(DB_jan<4.44,4.44,DB_jan),
  ...
  if (identical(badCount,badCount3)){
    write.csv(df.out,file=paste0(dataPath,y,"_fewsr_tower_input_final.csv"),row.names = F)
    write.csv(df.out.winter,file=paste0(dataPath,y,"_fewsr_tower_input_final_winterized.csv"),row.names = F)
...
#write.csv(missingEIA_id,file="C:/WU/missingWTdata2.csv",row.names=F)
