#'@title checkBadFuel
#'@description Find natural gas combined cycle (NGCC) units that report fuel types in \cr \cr
...
#'@param data.out data.frame with Plant.Code, bogen, Reported.Prime.Mover data in 1 or more columns
#'specified by `Reported.Prime.Mover_cols` and Fuel.Heat.(month).
#'@param Reported.Prime.Mover_cols vector of character strings indicating columns in which 
#'Reported.Prime.Mover data exists.
#'@return numeric vector of Plant.Codes with questionable Reported.Fuel.Type.Codes to be flagged
...
checkBadFuel<-function(data.out,Reported.Prime.Mover_cols){
  #define vector of bad fuel types
  badFuels<-c("BIT","AB","ANT","GEO","LIG","MSB","MSN","OBS","PC","PUR","RC",
              "SGC","SGP","SLW","SUB","TDF","WAT","WC","WDS","WH","WND")
  badPlants<-numeric(0)
  
# filter only NGCC plants
  Reported.Prime.Mover_cols<-Reported.Prime.Mover_cols[Reported.Prime.Mover_cols %in% names(data.out)]
  data.out<-data.out %>% dplyr::filter_at((dplyr::vars(dplyr::all_of(Reported.Prime.Mover_cols))), 
                                          dplyr::any_vars(. %in% c("CA","CT","CS","NGCC")))
  
  # find plants with bad fuel types
  for (p in unique(data.out$Plant.Code)){
    subdata<-data.out %>% dplyr::filter(Plant.Code==p)
    if (any(subdata$Reported.Fuel.Type.Code %in% badFuels)){
      subdata<-subdata %>% dplyr::filter(Reported.Fuel.Type.Code %in% badFuels)
      
      if (nrow(subdata)!=0){
        subdata<-dplyr::ungroup(subdata)
        subdata<-subdata %>% dplyr::select(dplyr::contains("Fuel.Heat"))
