import_bogen_data <- function(bogen)
{
   ...
   if(!is.data.frame(bogen) || any(!c('Plant.Code','Plant.Name','Boiler.ID','Generator.ID') %in% names(bogen))) 
  {
    stop("data must be a data.frame with columns 'Plant.Code','Plant.Name','Boiler.ID','Generator.ID' for this function to continue")
  }
  
  #Test check variable types
  test_that("Bogen data variable types are correct",
            {
              expect_true(is.character(bogen$Plant.Name),"Error wrong variable type. Plant.Name is a character string.")
