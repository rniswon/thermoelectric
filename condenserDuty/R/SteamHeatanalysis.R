...
#'@description calculates monthly steam heat by plant and boiler, for output file 
#'             final_steam_heat.csv \cr \cr
...
#'@return `steam_heat_results` data.frame monthly steam heat by plant and boiler
#'@export
#'
...
SteamHeatanalysis <- function(ST_fuel_heat_df, boilerDesignData, pub_efficiency, 
  ...
  init_steam_heat_results <- estEfficiency(ST_fuel_heat_df, boilerDesignData,page3)
  ...
  temp_steam_heat$boiler_efficiency<-ifelse(!is.na(temp_steam_heat$boiler_efficiency) & 
  ...
  if(use_published_efficiencies){
    ...
    missing_efficiency <- temp_steam_heat %>% filter(is.na(boiler_efficiency))
    missing_efficiency$boiler_efficiency[is.na(missing_efficiency$boiler_efficiency)] <- as.numeric(as.character(lookup(missing_efficiency$dom_fuel, pub_efficiency)))
    missing_efficiency[outIndex] <- as.data.frame(apply(missing_efficiency[monthindex], 2, calSteamHeat, boiler_efficiency = missing_efficiency$boiler_efficiency))
    steam_heat_results <- rbind(temp_steam_heat[!is.na(temp_steam_heat$boiler_efficiency),], missing_efficiency)
    
    eval(parse(text=paste0("steam_heat_results <- steam_heat_results %>% 
    ungroup() %>% 
    ...
    missing_efficiency <- temp_steam_heat %>% filter(is.na(boiler_efficiency))
    missing_efficiency$boiler_efficiency[is.na(missing_efficiency$boiler_efficiency)] <- as.numeric(as.character(lookup(missing_efficiency$dom_fuel, est.efficiencies)))
    missing_efficiency[outIndex] <- as.data.frame(apply(missing_efficiency[monthindex], 2, calSteamHeat, boiler_efficiency = missing_efficiency$boiler_efficiency))
    steam_heat_results <- rbind(temp_steam_heat[!is.na(temp_steam_heat$boiler_efficiency),], missing_efficiency)
  ...
  if (!page3){
    names(steam_heat_results)[names(steam_heat_results) %in% c("boiler_efficiency",
                                                               "boiler_total_fuel_heat",
                                                               "boiler_total_steam_heat")]<-c("efficiency",
                                                                                              "RPM_total_fuel_heat",
