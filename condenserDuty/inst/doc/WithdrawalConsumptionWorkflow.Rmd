---
title: "WithdrawalConsumptionWorkflow"
output: 
...
`Thermelectric/cd/TutorialFiles/UserControlCrosswalk2015.xlsx`. This
...
used by the program. If you open the `UserControlCrosswalk2015.xlsx`
...
Only columns `InputFile`, `InputSheetName`, and `InputColumnName` should be
edited. Editing any columns that begin with `script` will cause the
program to fail. All columns of any given `InputSheetName` will be
accessible, but *only* columns listed in `InputColumnName` will be
standardized and replaced by `scriptColnames.`
...
`UserControlCrosswalk2015.xlsx` spreadsheet and run the following
code:
...
the required data properly. Open the `UserControlCrosswalk2015.xlsx`
...
`UserControlCrosswalk2015.xlsx` from left to right, if problems are
found in a given column the function highlights them, and stops
checking other columns. This makes sense because, if columns to the
left are not found, everything to the right of that column would also
not be found.

The user should replace any fields highlighted in yellow, save and
close the `UserControlCrosswalk2015.xlsx` spreadsheet, and repeat the
following line until this is displayed in the console:
...
`UserControlCrosswalk2015.xlsx` spreadsheet with your working 2020
crosswalk.
...
`InputSheetDescription` column of the `UserControlCrosswalk2015.xlsx`
...
code). EIA reports associations between boilers and generators in the
`bogen` table from the `inputData.list`, however, there are many
boilers and generators that are missing from the EIA boiler-generator
association table. Thus, the `EIAplantAssociation` package was
developed.
...
(from [EIA-923](https://www.eia.gov/electricity/data/eia923/), **Page
1 Generation and Fuel Data**). If monthly data is available in
...
`Thermoelectrict/cd/TutorialFiles/nuke_combogencoo_key.csv`. However,
...
1. `sheet3_key` - Table that associates unique boilers to
   ...
   Page 3 table from which `boilerFuelData` is sourced.
2. `sheet4_key` - Table that associates unique generators to
   boiler-generator associations. It is named after the 2015 EIA-923,
   Page 4 table from which `generation.data` is sourced.
3. `missingCSCT` - Table output of the `checkMissingCSCT()` function
   which checks for `CS` and/or `CT` Reported.Prime.Movers that are
   present in `gen_fuel_data` (EIA-923, Page 1) from `inputData.list` but
   missing from `boilerFuelData` (EIA-923, Page 3) and/or `generation.data`
   (EIA-923, Page 4). Column naming is based on the page number of the
   data on the EIA-923 form. `gen_fuel_data` is reported at the plant
   level and therefore is not ideal for this type of analysis,
   however, if data exists in gen_fuel_data and not in `boilerFuelData`
   or `generation.data` then `gen_fuel_data` is used for the analysis. For
   NGCC units that fall into this category a boiler-generator association of
   `(Plant.Code)^NGCC` will be assigned.
4. `manualPlants` - Table of all plants flagged for manual
   boiler-generator association checks and columns for all manual
   flags indicated why the plant was selected for manual checks.
...
described in this input file
`Thermoelectric/cd/TutorialFiles/ColumnLegend_metafile.xlsx`.  Most
flags are simply informative, however, the highlighted flags have
...
These flags pick up questionable data that could indicate a incorrect
...
3. `flag_923bogenCTonly` - Indicates that in the only
   Reported.Prime.Mover in the generation.data and the boilerFuelData
   from the inputData.list is `CT`;
4. `flagNetGenWithoutFuelHeat` - Count of months where Net Generation>0
   but Fuel Heat<0 at the plant level;
5. `flagFuelHeat` - Indicates monthly Fuel Heat summed to the plant
   level by line in `boilerFuelData` from `inputData.list` is
   different in any one month from Fuel Heat summed by
   boiler-generator association to the plant level;
6. `flag_STassoc_CT_CS_CA` - Indicates a `ST` Reported.Prime.Mover has
...
the user that are available in the users environment for that
plant. An example file is stored here
`Thermoelectric/plant_association/compiled_ManualBogens.xlsx`.
...
1. `outputPath` - path (including file name) where compiled file should
   be saved;
2. `plantList` - unique vector of Plant.Codes to be included in the file
   (recommend all `manualPlants` from `bogen.out.list`);
3. `outTables` - list of named tables to be included in
   ...
   the file are completely up to the user to select
...
`auto_sheet4_key` tables is named `manualEdit`, if the user makes and
...
`manualEdit` column to `1` and fill in the `Plant.Code`, `Boiler.ID` (or
`Generator.ID`) and boiler-generator association columns.
...
from here `Thermoelectric/cd/TutorialFiles/combogencoo_cool_type.csv`.
...
checks, and it recommended that the user save the edited version with
each year analyzed and use the past analysis edited file with the year
closest to the current year being run as the "static" file starting
point by replacing the
`Thermoelectric/cd/TutorialFiles/combogencoo_cool_type.csv`. with the
version closest to your analysis year.
...
static `Thermoelectric/cd/TutorialFiles/combogencoo_key.csv` is used
to fill missing boiler-generator associations if boiler-generator
associations match `combogen` column from
`Thermoelectric/cd/TutorialFiles/combogencoo_key.csv`.
...
2. `manualBogencoo` - table with `Plant.Codes` flagged for manual cooling
   association checks. Plants are flagged for manual checks if the
   boiler-generator association does not appear in the static
   `Thermoelectric/cd/TutorialFiles/combogencoo_cool_type.csv` file and
   multiple cooling systems exist. Nuclear plants are not included;
3. `combogencoo_cooly_type` - table derived from the
   `Thermoelectric/cd/TutorialFiles/combogencoo_cool_type.csv`. file
   subset to only include non-nuclear plants;
4. `combogencoo_cooly_type_nukes` - table derived from the
   `Thermoelectric/cd/TutorialFiles/combogencoo_cool_type.csv`. file
   subset to only include nuclear plants.
...
is nearly identical to manual bogen checks. The same function is used
to generate a compiled file to edit (`compilePlantInfo_excel()`) and
apply the manual edits (`manualBogenReplace()`).
...
boiler-generator-cooling associations output by the `EIAplantAssociation`
package. The generalized workflow of functions used to calculate
condenser duty is given below.
...
`Thermolelectric/cd/TutorialFiles/nuke_combogencoo_key.csv` discussed
in "Section 4: Generate Cooling-System Groups" is loaded, and then two
static lists are defined.
...
and for all other boiler-generator associations,
`boilerFuelData` and `generation.data` are
used. The first step in the process is to calculate fuel heat which is
done using the `analyzeFuelHeat()` function.
...
4. `fuel_pct` is the fuel_specific_total_category/column name specified
   by `nameTotal`;
5. `dom_fuel` - applied according to the `calFuelDominance()` and
   `assignDominance()` functions. Fuel is considered dominant if
   `fuel_pct` > `dom_thrsh` argument which is set to `0.8`. If no dominant
   fuel reaches the `dom_thrsh` threshold, dominant fuel is set to
   `other`.
...
"realistic". `boiler_efficiency` must be >0.75 and <0.94. Boiler
efficiencies that fall outside this range are removed. Missing
boiler_efficiencies are replaced with values from the `pub_efficiency`
list according to dominant fuel type.
...
4. `fuel_pct_total_steam_heat` is `fuel_specific_total_steam_heat` /
...
4. `Net.Generation.Year.To.Date` is the sum of `Net.Generation.`(`month`) by
   row with negatives replaced by zeros.
...
output of the `analyzeFuelHeat()` function and utilizes the
`calNominalLoss()` function to assign nominal loss. For steam units,
nominal loss is defined as `fuel_heat_(month)*0.02`, and for NGCC
units nominal loss is defined as `fuel_heat_(month)*0.22`.
...
tables and combines the plant-level boiler-generator associations with
boiler-generator-level boiler-generator associations. The
